(function (Patient, Medication) {
    "use strict";
    Patient.Views.Condition = Patient.Views.Collection.extend({
        // app.messages.WARNING_DELETE_CONDITION
        count: 0,
        views: [],
        ...
        addOne: function(model) {
            ...
            if (model.get('patient_id')) {
                if (count == 0) {
                    ...
                    this.$("input[name=conditions][value="+hasRecords+"]").attr('checked', 'checked');
            ...
            var view = new Patient.Views.CollectionItem({model: model, template: _.template($('#condition-item').html()) });

            this.$('.entries').append(view.render().el);
            view.$('[name=start_date_formatted]').val(model.get('start_date_formatted'));
            $('#startdate-' + count).placeholder();
            ...
            var $searchInput = view.$('.condition');
            this.addAutocomplete($searchInput, 'condition', function(data, cb) {
            ...
            $searchInput.bind('autocompleteselect', function(event, ui) {
                var icd_type = ui.item.icd_type.toLowerCase(),
                        icd_selector = '#condition-' + count + '-' + icd_type;
