class PatientMedicalFacility < ApplicationRecord
  include PrivacyValidations
  self.sequence_name = :seq_patient_medical_facility
  
  # not sure whaty this is in both this and medical_facilty, lul
  enum medical_facility_type: {
    hospital: 'HOSPITAL'
  }

  enum patient_medical_facility_stat: {
    patient: 'PATIENT',
    preferred: 'PREFER' # yes this is the wrong speeeeelling, but it's in the db, so we run with it™
  }

  attr_readonly :patient_medical_facility_id, :patient_id, :create_user, :create_date
  # attr_accessible :record_order, :name, :phone, :url, :address_line1, :city, :state_province, :country, :postal_code,
  #   :address_line2, :address_line3, :medical_facility_type, :patient_medical_facility_stat, :privacy,
  #   :as => [:admin,:default]
  # attr_accessible :patient_id, :patient_medical_facility_stat,
  #   :as => :admin

  # Table relationships

  belongs_to :patient, :inverse_of => :patient_medical_facilities

  has_many :account_patients, :foreign_key => :patient_id, :primary_key => :patient_id 

  def self.hospitals_by_patient_id(patient_id)
    where(%{patient_id = ? AND medical_facility_type = 'HOSPITAL'}, patient_id)
  end
